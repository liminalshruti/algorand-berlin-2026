...
**Current demo beat:** ranked agents → x402 challenge (gap in red if charge > quote) → payment proof + automatic validation (verdict + reputation delta, hash-anchored) → re-run reroutes off the caught agent. _(Legacy router-settled `pay/validate` shim remains the fallback.)_
