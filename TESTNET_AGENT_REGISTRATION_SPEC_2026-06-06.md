...
`smart_contracts/identity_registry/contract.algo.ts:65`
...
- Agent Studio page: `public/studio.html` (`data-view="manage"`), engine `public/registry.js`.
...
- Chain context badge + app-ids: `public/arc8004.js` (`NET`, `APP`, `GENESIS`) and `registry.js::renderChainCtx`.
...
## 4. New module — `sandbox/lib/router/identity-onchain.ts`
...
1. **Network flip.** `public/router.js` `NETWORK` and `public/arc8004.js` `NET` → `"testnet"` (gate behind a single const so it's one edit). Explorer + chain-ctx badge then read TestNet.
2. **Real app-ids.** Replace `arc8004.js` `APP = {identity,reputation,validation}` mock `1001/1002/1003` with the deployed ids (inject via a small `public/chain-config.js` written at deploy time, or hand-edit tonight).
...
| `public/registry.js` | edit | UI | `LIVE_AGENTS` flag + live `fetch` register/list with mock fallback |
| `public/router.js` | edit | UI | `NETWORK="testnet"` |
...
- Editing `sandbox/bin/berlin-server.js` or `sandbox/lib/x402/*` (off-limits per `CLAUDE.md`).
