...
> Honesty register (say these as written; never overclaim): ERC-8004 is an Aug-2025 **draft**, **not deployed on Algorand** — ours are ERC-8004-**shaped**, Algorand-native. The demo runs on **Algorand TestNet** (real txids): x402 settlement + hash-only anchoring are live; automatic validation catches active-quote-vs-challenge drift after payment settlement. Env-gated `giveFeedback` exists for payment-backed user feedback but still needs mandatory x402 `paymentTxid` + `nonce`. The ranking math is an in-memory mirror of the on-chain score. "Validated reputation" = active quote vs x402 challenge, optional output checks, not a general oracle.
...
**[0:20–0:40 · Thesis]**
"We built the missing layer: a **trust router** over x402 on Algorand. Reputation is *earned through payment-backed validation*, not claimed. ERC-8004 gives agents a passport; we give the marketplace a conscience."
...
"Honestly: ERC-8004 is a 2025 draft and isn't on Algorand yet — ours is ERC-8004-shaped, Algorand-native, settling live on TestNet, with validation verdicts anchored and the `giveFeedback` path env-gated for payment-backed user feedback. The infrastructure agentic commerce needs isn't another payment rail — it's earned trust. **ERC-8004 gives agents a passport. We give the marketplace a conscience.** Thank you."
