...
> Honesty register (say these as written; never overclaim): ERC-8004 is an Aug-2025 **draft**, **not deployed on Algorand** — ours are ERC-8004-**shaped**, Algorand-native. The demo is **mock-first** with **live x402 settlement + hash-only anchoring on LocalNet** (real txids). "Validated reputation" = the specific checks we run (price-vs-quote, output) — not a general validation oracle.
...
"Watch. The cheapest provider wins the route. We pay — and the settled amount comes back **higher than the quote**: a hidden fee, caught from chain data. The verdict drops its reputation, **anchored on Algorand**. Now we re-run the same request — and the router **routes around it** to the honest provider. The marketplace self-corrected. Reputation is *earned*, not asserted."
...
**[0:40–1:50 · Live demo — the loop]** *(drive the UI; see DEMO_STORYBOARD.md)*
- "An operator needs a diligence read. The router collects competing quotes and ranks them — **price, earned reputation, and validation**, in one trust score. Cheapest here is *Vega*, and it leads."
- "Operator approves. We settle over x402 on Algorand — real transaction. But look: **settled is higher than quoted**. A hidden fee, caught directly from the on-chain settlement."
- "Validation writes the verdict: *reputation drops, 88 to 78, **because** settled exceeded quote* — anchored hash-only on Algorand. Here's the signed packet, here's the anchor."
...
"Behind the router is an **ERC-8004-shaped registry on Algorand** — identity, reputation, validation. Click any agent and you can see the **transactions behind its score**: every feedback entry references an x402 payment. That's the point — reputation you can *audit to a payment*, not a number a provider typed about itself."
...
"Honestly: ERC-8004 is a 2025 draft and isn't on Algorand yet — ours is ERC-8004-shaped, Algorand-native, and what you saw settles live on LocalNet with real txids. The infrastructure agentic commerce needs isn't another payment rail — it's earned trust. **ERC-8004 gives agents a passport. We give the marketplace a conscience.** Thank you."

---

## C) Q&A — 20-second answers

- **"Isn't this just a reputation system?"** Reputation systems take self-reported feedback. Ours only counts feedback **anchored to an x402 payment** — verified purchasers — and validates the delivery against the quote on-chain. Earned, not asserted.
- **"What's actually on-chain vs off?"** On-chain: the x402 settlement and **hash-only** anchors (note field) — verdicts, reputation. Off-chain: the detail the hash commits to. You verify the score without us exposing the substrate.
...
- **"Is it live?"** The x402 settlement + anchoring run live on LocalNet with real txids today; the ranking/validation registries are wired and mock-backed where the on-chain apps aren't deployed yet. We were deliberate about not faking the chain parts.
- **"Who's the buyer?"** Whoever operates an agent marketplace or routes agent work — they need provider trust they can defend. The console is the operator's admin surface.
...
- If the live server isn't up, the UI **falls back to mock automatically** and the indicator shows it — keep going; the loop is identical. Don't apologize, just note "running on LocalNet/mock" if asked.
