...
> **Precision register (say as confidence, not apology).** ERC-8004 is a 2025 **draft**, not on Algorand → ours is ERC-8004-**shaped**, Algorand-native. **Deployed + cross-linked on TestNet today:** Identity `764031067`, Reputation `764031363` (x402-coupled, payment-backed feedback), Validation `764031094` — code-hash verifiable. **Live in the demo:** x402 settlement, hash-only anchoring, automatic catch → reputation drop → reroute. **Proof path wired (router):** `/api/challenge → /api/payment-proof → /api/feedback` verify a real confirmed payment and write ValidationRegistry + ReputationRegistry `giveFeedback` — env-gated, payer-signer-only, hash-anchor fallback. **Proven on TestNet:** a full register → pay → review → validate audit trail across three independent wallets (`audit/LATEST.md`), every txid on the explorer. **On stage:** the live UI drives the router-settled shim and anchors hash-only; on-chain ranking is the next wire-up. "Validated reputation" = active quote vs x402 charge + future attestations — not a general oracle.
...
"Why is this infrastructure, not a feature? x402 is the settlement rail. ERC-8004 names the registries an agent economy needs — identity, reputation, validation — but it doesn't say what makes an entry *earned*. That discipline is the layer we built. And we built it on Algorand for one reason: the payment and its proof live on the *same ledger*. Proof-of-payment is native, not a foreign hash you have to trust — and sub-cent finality makes validating every call economical. A marketplace won't build this — they rank on their own take-rate; a neutral, chain-enforced trust layer is the wedge. These three registries are deployed and cross-linked on TestNet right now — here are the app-ids; verify the code hashes yourself. And we didn't just deploy them: we ran the full lifecycle on TestNet — register, pay, review, validate, across three independent wallets — every txid in our audit trail."
...
- **"What's on-chain vs off?"** On-chain (TestNet): three deployed, cross-linked registries; x402 settlement; hash-only anchors; and a full register→pay→review→validate audit trail (`audit/LATEST.md`). The proof path's ValidationRegistry + `giveFeedback` writes are wired and env-gated. The live stage UI uses the router's in-memory mirror for speed; on-chain ranking is the next wire-up.
...
- **"Is it live?"** Yes — TestNet, real txids, and audited: a confirmed register → pay → review → validate trail across three independent wallets (`audit/LATEST.md`). The proof-backed path — challenge → payment-proof → feedback, with on-chain validation + reputation writes — is wired in the router; surfacing it in the demo UI is the final step.
