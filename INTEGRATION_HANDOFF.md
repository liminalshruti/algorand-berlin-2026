...
- All router wire types live in `sandbox/lib/router/contract.ts` — import from there and coordinate before changing shared shapes.
...
- Routes → `sandbox/lib/router/routes.agents.ts` → inside `makeAgentRoutes(ctx)`
- Logic → `sandbox/lib/router/agents.ts`
...
**Target demo beat:** ranked agents → active quote pinned → agent x402 challenge asks more → payment settles for challenge → automatic validation drops reputation → re-run reroutes off the caught agent. Pitch script/deck/storyboard in `pitch/`.
