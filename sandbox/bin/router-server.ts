...
async function main() {
  ...
  app.post("/api/route", async c => {
    ...
    const withTrust = ranked.map(s => ({
      ...s,
      t: trustOf(s.p.quote, s.reputation, s.validation_rate)
    }));
  ...
  serve({ fetch: app.fetch, port: PORT }, () => {
    ...
    console.log("\n--- endpoints ---");
    console.log(
      "  POST /api/route { task, register }    (TEMP stub — Reza overrides)"
    );
