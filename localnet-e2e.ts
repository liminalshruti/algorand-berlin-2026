 ...
 * generated typed clients on a real algod node. Run: `tsx localnet-e2e.ts` (LocalNet must be up).
 */
import { AlgorandClient, Config } from '@algorandfoundation/algokit-utils'
import { ReputationRegistryFactory } from './smart_contracts/artifacts/reputation_registry/ReputationRegistryClient'
import { ValidationRegistryFactory } from './smart_contracts/artifacts/validation_registry/ValidationRegistryClient'
...
async function main() {
  ...
  await rep.send.giveFeedback({ sender: alice, args: { agentId: 1n, value: i128(5n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
  check('giveFeedback -> lastIndex == 1', (await rep.send.getLastIndex({ args: { agentId: 1n, client: alice.addr.toString() } })).return === 1n)
  await rep.send.giveFeedback({ sender: alice, args: { agentId: 1n, value: i128(7n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
  ...
  // distinct clients
  await rep.send.giveFeedback({ sender: bob, args: { agentId: 1n, value: i128(3n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
  ...
  // dec > 18 reverts
  await expectThrow('giveFeedback dec>18', () => rep.send.giveFeedback({ sender: alice, args: { agentId: 1n, value: i128(1n), dec: 19, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } }))
  ...
  await expectThrow('self-feedback by owner', () => rep.send.giveFeedback({ sender: owner, args: { agentId: 2n, value: i128(1n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } }))
  await rep.send.giveFeedback({ sender: alice, args: { agentId: 2n, value: i128(4n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
  ...
  await rep.send.giveFeedback({ sender: alice, args: { agentId: 3n, value: i128(10n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
  await rep.send.giveFeedback({ sender: bob, args: { agentId: 3n, value: i128(-32n), dec: 0, tag1: '', tag2: '', endpoint: '', feedbackUri: '', feedbackHash: ZERO32 } })
