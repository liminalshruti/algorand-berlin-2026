...
- **Pera Wallet** (`apps/web/wallet.js`, ESM module, no build): shared client across all pages. Loads `@perawallet/connect` + `algosdk@3` from esm.sh. `window.WALLET.{account,isConnected,connect,disconnect,signAndSend,payment}`; fires `wallet:change`/`wallet:ready`/`wallet:error` on `window`; any `[data-pera-connect]` element is an auto-labelled connect/disconnect toggle. Connected address mirrored in `localStorage` so pages share it.
  ...
  - **Local demo signer (router page only) NEW · 2026-06-07:** on `<body data-page="router">` (`router.html`), `wallet.js` auto-connects a *local* signer holding the public throwaway TestNet wallet from `apps/web/demo-wallet.testnet.json` — the SAME agent wallet the Claude `x402-demo` skill / MCP server pays from (`PAYER_MNEMONIC` in `.env.demo`, addr `24E3…`). It derives the key in-browser and signs/sends real TestNet txns (`signAndSend`/`payment`) with no Pera pairing. `window.WALLET` surface is unchanged (+ `WALLET.isLocal`), so `router.js` is unaffected (only the connect toast wording differs). Every other page (`mcp-sign`/marketplace/studio/contracts/admin) has no `data-page="router"`, so `LOCAL_MODE=false` and the real no-custody Pera flow is preserved. `demo-wallet.testnet.json` is a published valueless TestNet key — never reuse on MainNet.
