...
for app ids is also `public/deployed.testnet.json` (machine-readable).
...
- **Redeploy:** `npm run deploy:testnet` (orchestrator: `smart_contracts/deploy-testnet.ts`; idempotent via indexer)
